...
public class EditorActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        views =
                new Gson()
                        .fromJson(
                                FileUtil.readFromAsset("views.java", this),
                                new TypeToken<ArrayList<HashMap<String, Object>>>() {}.getType());
        layouts =
                new Gson()
                        .fromJson(
                                FileUtil.readFromAsset("layouts.java", this),
                                new TypeToken<ArrayList<HashMap<String, Object>>>() {}.getType());
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.menu_show_xml:
                                {
                                    String result =
                                            new XmlLayoutGenerator()
                                                    .generate(binding.editorLayout, true);
                                    Intent intent =
                                            new Intent(
                                                    EditorActivity.this, XmlPreviewActivity.class);
                            ...
                            case R.id.menu_drawable_manager:
                                {
                                    Intent intent =
                                            new Intent(
                                                    EditorActivity.this,
                                                    DrawableManagerActivity.class);
                                    intent.putExtra(
                                            DrawableManagerActivity.EXTRA_KEY_PROJECT, project);
                                    startActivity(intent);
                                    return true;
                                }
                        }
        ...
        public View getView(int pos, View buffer, ViewGroup parent) {
            ...
            Drawable icon =
                    AppCompatResources.getDrawable(
                            AppUIDesigner.getContext(),
                            InvokeUtil.getMipmapId(getItem(pos).get("iconName").toString()));
            icon.setBounds(0, 0, width, height);
            
            var iconColor = MaterialColors.getColor(bind.name, com.google.android.material.R.attr.colorOnSurface);
